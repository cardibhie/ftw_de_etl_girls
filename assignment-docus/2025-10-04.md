...
- **How do age bands affect dropout rates?**
- **Do students from certain regions or IMD bands perform**
- **Which assessments (Exam, TMA, CMA) have the highest failure rates?**
- **Which VLE activity types (forum, quiz, wiki, resource) lead to better performance?**

---
## :shield: Data Quality Checks
This week we added data quality monitoring for our key tables to ensure the analytics layer is reliable and actionable
If data is *missing* or *invalid*, the **DQ checks** flag these issues and make them visible. The DQ views help summarize the current state of the data at a glance
### What We Did:
- Added schema tests such as `not_null` and `accepted_values` for our tables
- Used `dbt build` to materialize **clean** (standardized raw data) and **mart** (fact/dim star schema) models
- Created **DQ summary views** to:
...
- Created **DQ anomalies views** to:
  - Drill down per row on data failing tests or rules
### Tables Covered:
- Dimensions: `dim_date`, `dim_assessment`, `dim_student_demographics`
